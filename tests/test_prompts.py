...
def test_ask_path_rejects_a_missing_path_then_accepts_a_real_one(answers, tmp_path, capsys):
    answers(str(tmp_path / "nope"), str(tmp_path))
...
def test_quit_raises_menu_exit(answers):
    answers("quit")
    with pytest.raises(MenuExit):
        read_menu_input("anything")


def test_zero_means_back_when_back_is_allowed(answers):
    answers("0")
    with pytest.raises(MenuBack):
        read_menu_input("anything", allow_back=True)


def test_zero_is_never_ordinary_input(answers, capsys):
...
def test_yes_no_understands_the_usual_spellings(answers, typed, expected):
    answers(typed)
    assert ask_yes_no("Continue?", True, allow_back=False) is expected


def test_empty_input_takes_the_default(answers):
    answers("")
...
def test_nonsense_is_rejected_and_asked_again(answers, capsys):
    answers("maybe", "y")
...
def test_csv_text_splits_and_trims(answers):
    answers(" japanese , commentary ")
    assert ask_csv_text_required("Titles") == ["japanese", "commentary"]


def test_csv_text_asks_again_when_empty(answers, capsys):
    answers("", "signs")
    assert ask_csv_text_required("Titles") == ["signs"]
    assert capsys.readouterr().out.strip() != ""


def test_csv_int_parses_a_list(answers):
    answers("1, 3,5")
    assert ask_csv_int_required("Indexes") == [1, 3, 5]


def test_csv_int_drops_a_non_number_and_keeps_the_rest(answers, capsys):
...
def test_csv_int_asks_again_when_nothing_parsed(answers, capsys):
    answers("two", "2")
    assert ask_csv_int_required("Indexes") == [2]
    assert "at least one stream index" in capsys.readouterr().out


def test_csv_int_confirms_an_index_the_scan_did_not_find(answers, capsys):
